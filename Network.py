...
class Network:
    def __init__(self, input_units, learing_rate):
        ...
        self._weights = {
            'encoder':tf.Variable(tf.random_normal([self._input_units, self._hidden_units])),
            'decoder':tf.Variable(tf.random_normal([self._hidden_units, self._input_units]))
        }

        self._biases = {
            'encoder':tf.Variable(tf.random_normal([self._hidden_units])),
            'decoder':tf.Variable(tf.random_normal([self._input_units]))
        }
    ...
    def encoder(self, x):
        layer = tf.nn.softmax(tf.add(tf.matmul(x,self._weights['encoder']),self._biases['encoder']))
